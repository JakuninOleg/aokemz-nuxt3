...
export function looksLikeGibberish(value: string): boolean {
  ...
  const vowelCount = (letters.match(new RegExp(VOWELS.source, "gi")) || [])
    .length;
...
export const contactLeadSchema = z.object({
  ...
  email: z
    ...
    .refine(
      (v) => !looksLikeGibberish(v.split("@")[0] || ""),
      "Проверьте email",
    ),
  message: z
    .string()
...
const optionalTechnicalField = (max: number) =>
  z
    .string()
    .trim()
    .max(max)
...
/** Optional fields from the technical selection form. */
export const technicalLeadSchema = z.object({
  equipment: optionalTechnicalField(120),
  machine: optionalTechnicalField(120),
  power: optionalTechnicalField(30),
  company: optionalTechnicalField(120),
});

export type TechnicalLeadDetails = z.output<typeof technicalLeadSchema>;

export function validateContactLead(data: unknown) {
  return contactLeadSchema.safeParse(data);
}

export function validateTechnicalLeadDetails(data: unknown) {
  return technicalLeadSchema.safeParse(data);
}

/** Validate a single field (for blur). Returns message or empty string. */
export function validateContactField(
  field: ContactLeadField,
  value: unknown,
): string {
